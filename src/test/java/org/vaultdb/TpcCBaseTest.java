...
import org.vaultdb.support.TpcCQueries;
...
public abstract class TpcCBaseTest  extends TestCase {
	...
	protected static final List<String> QUERIES_Delivery = TpcCQueries.QUERIES_Delivery;
	protected static final List<String> QUERIES_NewOrder = TpcCQueries.QUERIES_NewOrder;
	protected static final List<String> QUERIES_OrderStatus = TpcCQueries.QUERIES_OrderStatus;
	protected static final List<String> QUERIES_Payment = TpcCQueries.QUERIES_Payment;
	protected static final List<String> QUERIES_StockLevel = TpcCQueries.QUERIES_StockLevel;
...
//							"    CONSTRAINT s_phone_domain CHECK ((((\"substring\"((s_phone)::text, 1, 2))::integer = (s_nationkey + 10)) AND (\"substring\"((s_phone)::text, 3, 1) = '-'::text) AND (((\"substring\"((s_phone)::text, 4, 3))::integer >= 100) AND ((\"substring\"((s_phone)::text, 4, 3))::integer <= 999)) AND (\"substring\"((s_phone)::text, 7, 1) = '-'::text) AND (((\"substring\"((s_phone)::text, 8, 3))::integer >= 100) AND ((\"substring\"((s_phone)::text, 8, 3))::integer <= 999)) AND (\"substring\"((s_phone)::text, 11, 1) = '-'::text) AND (((\"substring\"((s_phone)::text, 12, 4))::integer >= 1000) AND ((\"substring\"((s_phone)::text, 12, 4))::integer <= 9999)))),\n" +
	...
	protected void setUp() throws Exception {
		SystemConfiguration.getInstance("tpcc").closeCalciteConnection();
		SystemConfiguration.resetConfiguration();
		SystemCatalog.resetInstance();
		ConnectionManager.reset();

		config = SystemConfiguration.getInstance("tpcc");
	...
	protected String readSQL(int queryNo) {
		return QUERIES_Delivery.get(queryNo-1); // zero-indexed
	}
